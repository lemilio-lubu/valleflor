...
export function SemanaSelector({ selectedId, onSelect }: Props) {
  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-2">
      {semanas.length === 0 && (
        <p className="text-carbon-400 text-sm font-mono">Sin semanas creadas</p>
      )}
      {semanas.map((s) => (
        <button
          key={s.id}
          ...
          className={`flex-shrink-0 flex flex-col items-center justify-center px-4 py-3 rounded-lg border transition-all duration-150 min-w-[80px] ${
            selectedId === s.id
              ? 'border-verde-600 bg-verde-900/30 text-verde-400 shadow-verde-sm'
              : 'border-surface-border bg-surface-overlay text-carbon-400 hover:border-carbon-700 hover:text-carbon-200'
          }`}
        >
