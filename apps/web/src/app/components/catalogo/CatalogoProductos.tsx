...
interface Producto { id: string; nombre: string; activo?: boolean; motivoBaja?: string | null; }
...
export function CatalogoProductos({ fincaId }: { fincaId: string }) {
  ...
  const { data: productos = [], isLoading: loadingProductos } = useQuery<Producto[]>({
    queryKey: ['productos', fincaId, mostrarInactivos],
    queryFn: () => api.get('/productos', { params: { fincaId, ...inactivosParam } }).then((r) => r.data),
  });
