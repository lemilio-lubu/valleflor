...
export function VariedadForm({ fincaId }: { fincaId: string }) {
  const qc = useQueryClient();
  const [editing, setEditing] = useState<Variedad | null>(null);
  const [nombre, setNombre] = useState('');
  const [selectedProductoId, setSelectedProductoId] = useState('');
  const [open, setOpen] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState<Variedad | null>(null);
  ...
  const save = useMutation({
    ...
    onSuccess: (_, data) => {
      qc.invalidateQueries({ queryKey: ['variedades', selectedProductoId] });
      toast.success(editing ? `Variedad "${data.nombre}" actualizada` : `Variedad "${data.nombre}" creada`);
      reset();
    },
  ...
  const remove = useMutation({
    mutationFn: ({ id }: { id: string; nombre: string }) => api.delete(`/variedades/${id}`),
    onSuccess: (_, { nombre }) => {
      qc.invalidateQueries({ queryKey: ['variedades', selectedProductoId] });
      toast.success(`Variedad "${nombre}" eliminada`);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={() => { reset(); setOpen(true); }} className="btn-ghost text-xs py-1.5">+ Nueva</button>
      </div>

      <div className="mb-3">
        <select className="input-field text-xs" value={selectedProductoId} onChange={(e) => setSelectedProductoId(e.target.value)}>
      ...
      {open && (
        <form onSubmit={(e) => { e.preventDefault(); save.mutate({ nombre, productoId: selectedProductoId }); }} className="mb-4 flex gap-2">
          <input
            ...
            {variedades.map((v) => (
              <tr key={v.id} className="table-row-hover border-b border-surface-border/30">
                ...
                <td className="px-4 py-2.5 text-right">
                  ...
                  <button onClick={() => setConfirmDelete(v)} className="text-carbon-300 hover:text-red-600 text-xs transition-colors">Eliminar</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {confirmDelete && (
        <ConfirmModal
          message={`¿Eliminar la variedad "${confirmDelete.nombre}"?`}
