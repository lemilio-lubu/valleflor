...
export const ConsolidadoDiario = forwardRef<ConsolidadoDiarioRef, Props>(
  function ConsolidadoDiario({ fincaId, responsableId, semana, anio, viewMode }, ref) {
    const { data: rows = [], isLoading } = useQuery<ConsolidadoDiarioRow[]>({
      queryKey: ['consolidado-diario', fincaId, responsableId, semana, anio],
      queryFn: () =>
        api.get('/consolidado/diario', { params: { fincaId, responsableId, semana, anio } }).then((r) => r.data),
    });
    ...
    const handleDownload = () => {
      ...
      const data = rows.map((r) => [
        ...
        ...DIAS.map((d) => { const v = r.dias[d]; return v ? (isCajas ? v.cajas : v.tallos) : 0; }),
    ...
    useImperativeHandle(ref, () => ({ download: handleDownload }), [rows, viewMode]);
