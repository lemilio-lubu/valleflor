...
export function ConsolidadoSemanal({ fincaId, responsableId, semana, anio }: Props) {
  const { data: rows = [], isLoading } = useQuery<ConsolidadoSemanalRow[]>({
    queryKey: ['consolidado-semanal', fincaId, responsableId, semana, anio],
    queryFn: () =>
      api
        .get('/consolidado/semanal', { params: { fincaId, responsableId, semana, anio } })
        .then((r) => r.data),
  });
        ...
        <table className="min-w-max w-full text-xs">
          <thead>
            <tr className="bg-surface-overlay border-b border-surface-border">
              ...
              <th className="table-th text-center text-dorado-400 min-w-[100px]">Cajas Est.</th>
              <th className="table-th text-center text-dorado-400 min-w-[100px]">Tallos Est.</th>
          ...
          <tbody>
            {rows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-center font-mono tabular-nums text-dorado-500">
          ...
          <tfoot>
            <tr className="border-t-2 border-surface-border bg-surface-overlay">
              ...
              <td className="px-3 py-2.5 text-center font-mono tabular-nums font-bold text-dorado-400">
                {totals.cajasEstimadas.toFixed(2)}
              </td>
              <td className="px-3 py-2.5 text-center font-mono tabular-nums font-bold text-dorado-400">
                {totals.tallosEstimados.toFixed(2)}
              </td>
              <td className="px-3 py-2.5 text-center font-mono tabular-nums font-bold text-agro-500">
